...
def reward_fn(properties, generated_smiles):
    # 支持单个字符串和列表输入
    if isinstance(generated_smiles, str):
        smiles_list = [generated_smiles]
    else:
        smiles_list = generated_smiles

    if properties == 'druglikeness':
        vals = batch_druglikeness(smiles_list)
    elif properties == 'solubility':
        vals = batch_solubility(smiles_list)
    elif properties == 'synthesizability':
        vals = batch_SA(smiles_list)
    else:
        raise ValueError(f"Unknown property: {properties}")

    # 如果输入是单个字符串，则只返回第一个值，否则返回整个列表
    return vals[0] if isinstance(generated_smiles, str) else vals
...
def ads(x, a, b, c, d, e, f, dmax):
    # 如果 x 是 RDKit 的 vector 或 list，取长度作为计数
