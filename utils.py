...
def distribution(real_file, gan_file, wgan_file):
	...
	for name in ['QED Score', 'SA Score', 'logP Score']:
		...
		sns.distplot(real_scores, hist = False, kde_kws = {'shade': True, 'linewidth': 1}, label='ORIGINAL')
		sns.distplot(gan_scores, hist = False, kde_kws = {'shade': True, 'linewidth': 1}, label='STGAN')
		sns.distplot(wgan_scores, hist = False, kde_kws = {'shade': True, 'linewidth': 1}, label='ST(W)GAN')
